...
### v1.5.0-fix3 — 2026-05-30 (fix blocker mode application — `display: revert` collapsed custom elements to invisible, Blur/Dim produced no visible effect)
- **Bug report.** In v1.5.0-fix2 real-Chrome testing, mode switching was visibly broken: `Toggle ON` worked for `home-feed`, `shorts`, `watch-sidebar` (Hide mode hid the elements), but switching the dropdown to Blur/Dim/Grayscale/Hover-only blur produced NO visible change. Storage was correct (`chrome.storage.local.get(["blockerModes"])` returned the right value), but the DOM wasn't reflecting it.
- **Root cause: `display: revert !important` in mode override rules** (`content/styles.css`, 30 occurrences across every blocker × {blur, dim}). CSS spec: `display: revert` cascades to the **user-agent stylesheet** default. For autonomous custom elements (`<ytd-rich-grid-renderer>`, `<ytd-shelf-renderer>`, `<grid-shelf-view-model>`, every target of these rules), the HTML spec defines the UA default as **`display: inline`**. So when the user picked Blur, the mode rule fired, did `display: revert` → element became `inline` → block-level children collapsed → element rendered as invisible → `filter: blur(8px)` then applied to nothing the user could see. Same for Dim. The original v1.4.19 author intended `revert` to "undo display:none from the base rule"; for div-derived elements it would have worked (UA default = block), but YouTube's custom elements aren't HTML standards-defined — UA default for autonomous customs is `inline`.
- **fix3 — `display: revert` → `display: block`** across all 30 mode-override rules in `content/styles.css`. Verified by inspection that every targeted element (ytd-rich-grid-renderer, ytd-shelf-renderer, ytd-watch-flexy children, ytd-comments, ytd-live-chat-frame, ytd-merch-shelf-renderer, ytd-radio-renderer, etc.) is a block-level container by design. `display: block` matches what the YT stylesheet computed for these elements, restoring them to layout flow so the filter/opacity is visible. The thumbnails-specific mode rules (which already use `opacity: 1` to override the base `opacity: 0`) were not affected by this bug and are unchanged.
- **Caveat on thumbnails Hide mode**. The user also reported "Hide thumbnails does nothing visible" — which CANNOT be explained by `display: revert` (the thumbnails base rule uses `opacity: 0`, not `display: none`). The Phase 1 health log shows `ytd-thumbnail img` matches 3 elements on the user's YT homepage, so the SELECTOR fires; the issue is likely that YouTube has moved visible thumbnails into closed Shadow DOM or replaced `<img>` with `<picture>`/CSS-background. Global rules can't pierce closed Shadow DOM. **fix3 does NOT attempt to fix this** because I can't verify the current YT thumbnail DOM without real Chrome access; the user's `cf_health_log` from a real session is the next diagnostic — if `ytd-thumbnail img` matches 0 on the homepage post-fix3 with thumbnails ON, that's selector rot and needs an updated selector. The fix3 test suite at least locks down the class-application correctness so the user can isolate it.
- **New regression sentinel: `tests/blocker-mode-application.js` (166/166).** Asserts the (blocker, mode) → CSS-rule contract that fix3 just restored. Sections:
  1. Every (blocker, mode) combo has at least one `body.cf-block-<id>.cf-mode-<id>-<mode>` rule in `content/styles.css` — 34 combos checked (16 non-autoplay blockers × 2 modes, plus thumbnails' extra +2).
  2. **`display: revert` is NEVER present in any mode rule body** — the exact regression sentinel for fix3. If a future styles.css edit reintroduces it, this fails loud.
  3. Every mode rule body sets at least one of `filter:`, `opacity:`, or `display: (block|grid|flex|inline-block)` so the user actually sees an effect (`pointer-events: none` alone doesn't count).
  4. `_ALL_MODES` in `content.js` enumerates every mode value that appears in styles.css cf-mode classes (so `applyBlockers` tear-down cleans all of them).
  5. Popup `_BLOCKER_MODE_OPTIONS._core` has `[hide, blur, dim]` and `_BLOCKER_MODE_OPTIONS.thumbnails` has all 5 modes (regression sentinel against future popup-table desync).
  6. Thumbnails blur/grayscale rules set `opacity: 1` + the appropriate filter (verifies the opacity-undo-base-rule pattern).
  7. NO blocker has an explicit `cf-mode-X-hide` rule (hide is the base default; an explicit hide rule would be redundant and confusing).
- **Anti-scope-creep guarantees.** Only `content/styles.css` was modified — 30 token-level changes from `display: revert` to `display: block`, plus a paragraph in the rule-header comment explaining the fix. No JS, no HTML, no other CSS, no manifest changes (beyond version bump). Cloudflare Worker, ExtPay SDK, pricing, grandfather, cf_stats, Phase 1/2/3 + fix1/fix2 — all unchanged.
- **All 24 existing suites stay green.** Grand total: **1351 pass / 0 fail** (+166 from v1.5.0-fix2's 1185).
- **Manifest.** `version: "1.5.0.4" → "1.5.0.5"`. `version_name: "1.5.0-fix2" → "1.5.0-fix3"`. Zip: `dist/cleanfeed-v1.5.0-fix3.zip`. All 12 `_locales/` folders intact.
- **Diff summary.** `content/styles.css` (+12/-1: 30 `display: revert` → `display: block` token replacements + paragraph in mode-rules header comment), `manifest.json` (2 lines), `README.md` (this entry), `tests/blocker-mode-application.js` (new, ~200 lines, 166/166). Zero changes to `content/content.js`, `content/blockers.js`, `content/selectors.js`, `content/health-log.js`, `popup/`, `options/`, `_locales/`, `onboarding/`, `lib/`, `icons/`, `background.js`, `build.py`.
- **Manual verification plan (fix3).**
  1. Extract: `mkdir -p ~/cleanfeed-v150-fix3-unpacked && python3 -m zipfile -e ~/workspace/cleanfeed/dist/cleanfeed-v1.5.0-fix3.zip ~/cleanfeed-v150-fix3-unpacked/`. **Remove the previous CleanFeed entry first** → Load unpacked → pick the fix3 folder.
  2. Visit youtube.com homepage with all 17 blockers toggled ON.
  3. **For home-feed**: pick Hide → grid disappears. Pick Blur → grid renders blurred (block-level layout restored, filter:blur visible). Pick Dim → grid renders at 15% opacity. Hover the grid → blur/dim lifts. (Pre-fix3: Blur and Dim picked but grid was invisible because display:revert → inline.)
  4. **For shorts**: pick Blur → shorts shelves visible but blurred. Pick Dim → dimmed.
  5. **For watch-sidebar** on a /watch page: Blur → sidebar visible but blurred. Dim → dimmed.
  6. **For thumbnails**: if Hide doesn't work in real Chrome, that's the YT-Shadow-DOM / `<picture>` migration issue separate from fix3. Run `chrome.storage.local.get(["cf_health_log"], console.log)` after browsing to confirm `ytd-thumbnail img` is still matching (if matches > 0 but no visible effect, YT changed thumbnail rendering and a separate selector update is needed).
  7. Confirm `document.body.classList` on a YT tab includes `cf-block-X` AND `cf-mode-X-<mode>` for every active blocker after switching modes.
...
- **Thumbnail anti-clickbait variants** (`content/styles.css` + `popup/popup.js` + `content/content.js`). The PRO "Hide thumbnails" blocker's mode dropdown gains two new options on top of the v1.4.19 hide/blur/dim trio:
  - **Grayscale** — `filter: grayscale(100%) !important`. Desaturates without obscuring; you still see what the thumbnail is, minus the engagement-bait colour pop.
  - **Hover-only blur** — `filter: blur(8px) !important` + `transition: filter 150ms ease` + `:hover { filter: none }`. Inverted variant of the existing Blur (sharp default, blurs on un-hover).
  Both variants are scoped to the thumbnails blocker only; `_BLOCKER_MODE_OPTIONS` keeps the other 16 blockers at the three-option core list. `applyBlockers` tear-down loop iterates `_ALL_MODES = ["hide","blur","dim","grayscale","hover-blur"]` so switching between any two modes leaves no stale class on body.
- **Homepage redirect destinations** (extends the v1.4.19 "Open YouTube on Subscriptions" toggle). New storage key `cf_homepage_destination` accepts:
  - `"subscriptions"` (default, backward-compat with v1.4.19)
  - `"library"` → `/feed/library`
  - `"history"` → `/feed/history`
  - `{ type: "playlist", url }` → raw URL passthrough
  - `{ type: "channel", handle }` → `/@handle` (or full URL passthrough)
  - `"blank"` → `chrome-extension://[id]/onboarding/blank.html`
  The popup toggle label is now "Open YouTube on a calmer page" (broader scope). Destination + per-type inputs live in a new options.html section. Empty playlist URL or empty channel handle gracefully fall back to `/feed/subscriptions`.
- **Blank prompt page** (`onboarding/blank.html` + `onboarding/blank.js`). Minimal calm-prompt page: heading "What did you come here to do?", a freeform self-prompt text input (not stored), two buttons. "Continue to YouTube" sets `cf_skip_next_homepage_redirect: true` via `chrome.storage.local.set` and navigates to `https://www.youtube.com/`; the content script clears the flag and skips the redirect for exactly that nav. Defensive fallback: if `chrome.storage.local.set` fails, the URL `?cf_bypass=1` query is appended — `_isBareHomepage` recognises this and also skips the redirect. "Close tab" calls `window.close()`. Pure local; no analytics. Inline styles only (works under strict CSP).
- **Regex matching for keywords & channels.** Storage shape generalised:
  - `hiddenKeywords` was `array<string>` → now `array<{pattern, isRegex}>`. Legacy strings normalise to `{pattern, isRegex:false}` on read. The matcher caches compiled regexes by pattern string (rebuilt on `storage.onChanged`) so the per-tick cost matches v1.4.0 baseline. Invalid regex patterns are silently skipped at match time; options.js surfaces "⚠ invalid" indicator next to the row in real time as the user types.
  - `blockedChannels` gains optional `isRegex` per entry. When true, both `handle` and `name` are tested against the pattern (`i` flag). Compiled regexes are cached on the entry object (`__rx`) for the lifetime of the array; storage.onChanged delivers a fresh array which forces re-compile next call.
  - Dedupe operates in two namespaces — substring "react" and regex "react" are different conceptual entries and both survive normalisation.
- **Options keyword UI rewrite.** The v1.4.0 textarea + Save button is replaced with a row-based list. Each row: `[pattern input]` `[☐ regex]` `[⚠ invalid indicator when applicable]` `[remove]`. + "Add keyword" button. Save fires on blur of input / Enter / checkbox change — no separate Save click. The 200-keyword cap is preserved. Options channel-blocklist rows gain the same `[☐ regex]` `[⚠]` controls; the right-click "Block this channel" path remains the only add route.
- **Defensive scaffolding preserved.** `cf_homepage_destination` + `cf_skip_next_homepage_redirect` defaults seeded in `background.js` install + `_migrateForV140` migration paths. content.js loads both; `cf_homepage_destination` defaults to `"subscriptions"` on absent/garbage; `cf_skip_next_homepage_redirect` defaults false. Existing v1.4.19+ users keep the same behavior — toggle still redirects to /feed/subscriptions because that's the default destination.
- **Anti-scope-creep guarantees.** No Watch Time Caps, no Schedule-Based Blocking, no Settings Sync, no Settings Backup — those are explicitly excluded from v1.5.0. Cloudflare Worker untouched. ExtPay SDK + boot logging (v1.4.21-fix2) unchanged. Pricing model + grandfather logic + cf_stats analytics unchanged. Phase 1's `selectors.js` + `health-log.js` + Diagnostic info UI unchanged. The 17 blockers' selector arrays still match v1.4.22 byte-for-byte (`tests/selectors-fallback.js`'s V1422_PRIMARY snapshot continues to pass).
- **New regression sentinels:**
  - `tests/thumbnail-variants.js` (38/38) — five-mode allowlist for thumbnails; other blockers stay restricted; `_ALL_MODES` enumerates every mode class; tear-down/re-apply leaves no stale class for grayscale→hover-blur, hover-blur→grayscale, or blur→grayscale; dropdown options.thumbnails has 5 options in canonical order; `_BLOCKER_MODE_OPTIONS._core` stays 3.
  - `tests/regex-keywords.js` (47/47) — legacy substring still works; v1.5.0 shape with isRegex:false equivalent to legacy; regex matches with `i` flag (^anchor, alternation, `\\b` word boundaries); substring + regex coexist; invalid regex silently skipped; substring "react" and regex "react" dedupe in different namespaces; empty/malformed entries pruned; 200-entry cap holds; channel blocklist exact-match (legacy), regex on handle, regex on name, invalid channel regex silently skipped, mixed entries coexist; normalize idempotency.
  - `tests/homepage-redirect.js` extended (33 → 55, +22) — default destination = subscriptions; explicit subscriptions/library/history route to right URL; blank → chrome-extension:// URL; playlist URL passthrough + empty fallback; channel handle/URL normalisation + empty fallback; `?cf_bypass=1` query bypass; one-shot bypass flag clears after firing; destination switch mid-session; anti-loop guard when target equals current path.
- **All 19 v1.5.0-phase1 suites stay green.** Grand total: **863 pass / 0 fail** (+107 from v1.5.0-phase1's 756).
- **Manifest.** `version: "1.5.0.0" → "1.5.0.1"`. `version_name: "1.5.0-phase1" → "1.5.0-phase2"`. Zip: `dist/cleanfeed-v1.5.0-phase2.zip`. All 12 `_locales/` folders intact.
- **Diff summary.** `content/content.js` (+105/-25: thumbnails 5-mode allowlist, _ALL_MODES tear-down loop, _normalizeKwEntry + _normalizedKwList + _kwMatcherCache + applyKeywordBlocks regex path, _channelMatcher with compiled __rx cache, _resolveHomepageDestinationURL + maybeRedirectHomeToSubs destination dispatch + one-shot bypass + `?cf_bypass=1` recognition, storage.onChanged hooks for cf_homepage_destination/cf_skip_next_homepage_redirect/blockerModes invalidation), `content/styles.css` (+45 lines: grayscale + hover-blur thumbnail rules for ytd-thumbnail/yt-image/.yt-thumbnail-view-model + ytmusic + yt-img-shadow), `popup/popup.js` (+30: _BLOCKER_MODE_OPTIONS table, thumbnail-only 5-option list, popup toggle label update), `options/options.html` (+30: Homepage destination section, keyword list rewrite, regex per-row in blocked list), `options/options.js` (+220: renderHomepageDestination + onHomepageDestinationChange + load/onChanged wiring, renderKeywords + _renderKeywordRow + saveKeywordRow + removeKeywordRow + addKeywordRow + _normalizeKws + _isValidRegex, toggleChannelRegex), `options/options.css` (+40: cf-keyword-list, cf-kw-row, cf-kw-regex, cf-kw-warn, cf-kw-remove), `background.js` (+8: cf_homepage_destination + cf_skip_next_homepage_redirect install defaults + migrations), `onboarding/blank.html` (new, ~110 lines: prompt page), `onboarding/blank.js` (new, ~50 lines: Continue + Close handlers), `manifest.json` (2 lines), `README.md` (this entry), `tests/thumbnail-variants.js` (new), `tests/regex-keywords.js` (new), `tests/homepage-redirect.js` (+170 lines, +22 assertions).
- **Manual verification plan (Phase 2).**
  1. Extract: `mkdir -p ~/cleanfeed-v150p2-unpacked && python3 -m zipfile -e ~/workspace/cleanfeed/dist/cleanfeed-v1.5.0-phase2.zip ~/cleanfeed-v150p2-unpacked/`. Remove old extension + Load unpacked.
  2. **Thumbnail variants** — popup toggle "Hide thumbnails" ON, mode dropdown now has 5 options. Pick Grayscale → thumbnails on youtube.com/ desaturate. Pick Hover-only blur → thumbnails blur, sharpen on mouseover. Switch back to Hide → blur/grayscale gone, opacity:0 takes over.
  3. **Homepage destinations** — popup toggle "Open YouTube on a calmer page" ON. In options, "Homepage destination" dropdown:
     - Subscriptions feed (default) → youtube.com/ lands on /feed/subscriptions.
     - Library → /feed/library.
     - History → /feed/history.
     - A specific playlist → paste a playlist URL → youtube.com/ lands there.
     - A specific channel → `@kurzgesagt` → /@kurzgesagt.
     - A blank page → youtube.com/ shows the CleanFeed "What did you come here to do?" prompt. Click "Continue to YouTube" → tab goes to youtube.com/ and shows the actual homepage (one-shot bypass). Refresh the tab → redirect resumes.
  4. **Regex keywords** — options "Hidden keywords" section. Click "+ Add keyword", type `^Top \d+`, check "regex". Visit youtube.com → "Top 10" videos are hidden. Type an invalid regex like `[broken` and tick regex → "⚠ invalid" indicator appears; no crash. Untick regex → indicator disappears, substring matching resumes.
  5. **Regex channels** — right-click a video on YouTube → "Block this channel with CleanFeed". Open options → "Blocked channels" — that channel appears with a per-row "regex" checkbox. Tick it; the handle is now treated as a regex pattern (`i` flag). Useful for "block all variants of this channel name pattern".

### v1.5.0-phase1 — 2026-05-29 (self-healing selector system + diagnostic log — defensive infrastructure, no user-visible behaviour change)
- **Phase 1 of v1.5.0.** Pure infrastructure — popup, options pricing copy, dashboard, blocker behavior on healthy YouTube pages are byte-identical to v1.4.22. The goal is to make "this blocker stopped working" reports debuggable without asking users to inspect DevTools.
- **New: `content/selectors.js`.** Single source of truth for every DOM selector used by every blocker. Shape: `SELECTORS[blockerId] = { primary: [...], fallbacks: [[...], [...]] }`. v1.5.0-phase1 populates `primary` byte-identically to v1.4.22's inlined `selectors` arrays in `content/blockers.js`; `fallbacks` is an empty array on all 17 blockers and gets populated as real-Chrome misses surface. Exposed via `window.__cleanfeed_selectors`.
- **Refactored: `content/blockers.js`.** Each blocker's `selectors` field is now a `get` accessor that returns the flattened `[primary..., ...fallbacks]` chain via a `selectorsFor(id)` helper. Every consumer (counting pass, stats, JS-handler blockers) gets the same value as v1.4.22 because fallbacks are empty. When fallbacks land in a future patch, the chain extends transparently.
- **New: `content/health-log.js`.** Public API: `recordSelectorMatch(blockerId, selectorIndex, matchCount)`, `recordSelectorMiss(blockerId)`, `markPageNav()`, `exportLog()`, `clearLog()`. Storage: `chrome.storage.local.cf_health_log`, ring buffer max 50 entries; oldest evicted on overflow. Entry shape: `{ kind: "match"|"miss", blockerId, ts, url, ua_short, selectorIndex?, matchCount? }`. URL is normalised to `origin + pathname` (no query/hash) so video IDs and auth tokens never enter the log. `ua_short` is `"Chrome <major> <platform>"` — enough to disambiguate "Chrome 122 on Linux" from older versions without leaking exact patch numbers. Throttle: each blocker fires AT MOST one miss and one match per `markPageNav()` call (i.e. once per YT navigation) — without throttling, a single broken selector would burn through the ring buffer in seconds. Writes are serialised through a promise chain to prevent read-modify-write races under rapid applyBlockers ticks.
- **Wired: `content/content.js`.** The existing `countBlockedElements` counting pass now tracks the first-hit selector index per blocker per nav, and calls `recordSelectorMatch` or `recordSelectorMiss` accordingly. `maybeNavReset` invokes `markPageNav()` so each YT navigation gets a fresh recording window. Blockers with empty selector chains (autoplay — JS-only) skip health-log entirely. The hide pipeline (CSS-class-driven `body.cf-block-{id}` rules in `styles.css`) is unchanged.
- **`manifest.json` content-script load order.** `selectors.js` and `health-log.js` are listed BEFORE `blockers.js` + `content.js` so the window globals are populated before any consumer dereferences them.
- **New: Options "Diagnostic info" panel.** Expandable `<details>` section at the bottom of options.html (above the footer). When opened, lazy-loads the log from `chrome.storage.local.cf_health_log` and displays as pretty JSON (newest first) in a read-only textarea. Two buttons: "Copy to clipboard" (calls `navigator.clipboard.writeText` with a graceful fallback message if blocked) and "Clear log" (confirmation dialog + `chrome.storage.local.remove`). Pure local — no network calls. ~99% of users will never open this section; the lazy-load avoids a storage round-trip on every options-page render.
- **Anti-scope-creep guarantees.** Cloudflare Worker untouched. ExtPay SDK + boot logging + watchdog (v1.4.21-fix2) preserved verbatim. Pricing model + grandfather logic + cf_stats analytics + Pro gating unchanged. The 17 blockers' selector arrays match v1.4.22 byte-for-byte (asserted by `tests/selectors-fallback.js`'s V1422_PRIMARY snapshot). No new permissions.
- **New regression sentinel: `tests/selectors-fallback.js` (107/107).** Covers: SELECTORS object exists with all 17 ids + primary/fallbacks arrays each; primary verbatim from v1.4.22 (9-blocker snapshot); flat chain equals primary when fallbacks empty; fallback-engagement test (synthetic mutation); `recordSelectorMiss` throttled to once per nav per blocker; `markPageNav` resets dedupe; `recordSelectorMatch` also throttled; mixed misses + matches accumulate independently; `exportLog` returns valid JSON; empty storage → `"[]"`; ring buffer caps at 50 (60 nav events → 10 oldest evicted); `clearLog` wipes + resets dedupe; URL normalisation strips query/hash; bad `blockerId` (empty/null/undefined) silently ignored.
- **Tests.** All 19 existing suites stay green: badge-count 12, blocker-modes 32, cf-stats-migration-real-chrome 49, first-install-race 13, grandfather-migration 52, homepage-redirect 33, license-redeem 62, migration-dryrun 46, onToggle-rapid-click 13, pause-rapid-click 15, show-comments-persist 58, stats-autoplay-counter 24, stats-blocker-counter 20, stats-no-double-count 7, subs-feed-cleanup 29, subscribe-button-flow 71, subscription-sync 45, upgrade-card-states 47, youtube-music-exempt 21. Grand total: **756 pass / 0 fail** (+107 from v1.4.22's 649).
- **Manifest.** `version: "1.4.22.0" → "1.5.0.0"`. `version_name: "1.4.22" → "1.5.0-phase1"`. Zip: `dist/cleanfeed-v1.5.0-phase1.zip`. All 12 `_locales/` folders intact.
- **Diff summary.** `content/selectors.js` (new, ~170 lines), `content/health-log.js` (new, ~120 lines), `content/blockers.js` (-220 / +90: each blocker's `selectors` field becomes a getter; `selectorsFor` helper; SEL alias), `content/content.js` (+40: health-log hooks in countBlockedElements + markPageNav in maybeNavReset), `manifest.json` (one line: content_scripts.js order), `options/options.html` (+15: Diagnostic info section), `options/options.js` (+95: loadDiagnosticLog + copyDiagnosticLog + clearDiagnosticLog + init wiring), `options/options.css` (+18: cf-diag-details summary + cf-diag-log textarea), `tests/selectors-fallback.js` (new, ~340 lines), `README.md` (this entry). Zero changes to `popup/`, `_locales/`, `lib/`, `icons/`, `build.py`, `background.js`.
- **Manual verification plan (Phase 1).**
  1. Extract: `mkdir -p ~/cleanfeed-v150p1-unpacked && python3 -m zipfile -e ~/workspace/cleanfeed/dist/cleanfeed-v1.5.0-phase1.zip ~/cleanfeed-v150p1-unpacked/`. Load unpacked.
  2. **Healthy-page behaviour unchanged.** Open youtube.com → homepage feed hidden as in v1.4.22. Open a watch page → sidebar hidden, comments hidden, end-screen suppressed exactly as before. Toggle blockers in the popup → identical behaviour.
  3. **Health-log accumulates.** Open SW DevTools (or any extension page DevTools) and run `chrome.storage.local.get(["cf_health_log"], console.log)`. After 1-2 YT navigations, the array should contain ≤17 `kind:"match"` entries (one per active blocker that had at least one selector hit on the current page) — `blockerId`, `selectorIndex: 0`, `matchCount > 0`, `url: "https://www.youtube.com/<path>"` (no query string), `ua_short: "Chrome 122 Linux"` (or your version).
  4. **Diagnostic info UI works.** Open options page → scroll to the bottom → click "Diagnostic info (for bug reports)" to expand. Textarea populates with the same JSON. Click "Copy to clipboard" → success status appears. Click "Clear log" → confirmation prompt → JSON resets to `[]`.
  5. **Miss simulation.** Run `chrome.storage.local.set({cf_health_log: [{kind:"miss", blockerId:"home-feed", ts: Date.now(), url:"https://www.youtube.com/", ua_short:"Chrome 122 Linux"}]})` in DevTools and reopen the Diagnostic info section — confirm the synthetic miss displays. (Real misses won't appear until/unless YouTube redesigns a section we currently match.)
